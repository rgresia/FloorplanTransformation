...
function utils.getNumberMap()
   if not utils.numberMap then
      ...
      numberMap.labels['office'] = 5
      numberMap.labels['closet'] = 6
      numberMap.labels['balcony'] = 7
      numberMap.labels['corridor'] = 8
      numberMap.labels['dining_room'] = 9
      numberMap.labels['stairs'] = 10
...
function utils.keyMap()
   ...
   keyMap['S'] = {'icons', 'sofa'}
   keyMap['X'] = {'icons', 'cabinet'}
   keyMap['B'] = {'icons', 'bed'}
   keyMap['T'] = {'icons', 'table'}
   keyMap['D'] = {'icons', 'desk'}
   keyMap['C'] = {'icons', 'chair'}
   keyMap['F'] = {'icons', 'refrigerator'}
   keyMap['M'] = {'icons', 'TV'}
   for i = 1, 10 do
...
function utils.reconstructFloorplan(pointcloud)
   predictionDir = '../PointCloudSegmentation2/'
   pointcloudFilename = predictionDir .. '/test/pointcloud.txt'
   print(pointcloudFilename)
   local pointcloudFile = io.open(pointcloudFilename, 'w')
   for pointIndex = 1, pointcloud:size(1) do
      point = pointcloud[pointIndex]
      pointcloudFile:write(point[1] .. '\t' .. point[2] .. '\t' .. point[3] .. '\n')
   end
...
function utils.checkSanity(floorplan, representation, lineWidth)
   ...
   for mode, items in pairs(representation) do
      if #items == 0 and mode ~= 'points' and mode ~= 'icons' then
         print('no ' .. mode)
         return false
      end
   end
   ...
   for roomIndex, labels in pairs(roomLabelsMap) do
      if #labels == 0 and roomIndex ~= backgroundRoomIndex then
	 print('room ' .. roomIndex .. ' has no label')
	 return false
      elseif #labels > 1 then
